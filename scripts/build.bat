...
cmake -S %REPO_DIR% -B %SCPP_LOGGER_BUILD_DIR% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% || exit /b !ERRORLEVEL!
echo ----- SCPPLogger CMake configuration done -----

echo ----- Building SCPPLogger in %SCPP_LOGGER_BUILD_DIR% -----
cmake --build %SCPP_LOGGER_BUILD_DIR% --config %BUILD_TYPE% || exit /b !ERRORLEVEL!
echo ----- SCPPLogger build done -----

echo ----- Installing SCPPLogger in %SCPP_LOGGER_BUILD_DIR% -----
cmake --install %SCPP_LOGGER_BUILD_DIR% --config %BUILD_TYPE% || exit /b !ERRORLEVEL!
...
cmake -S %EXAMPLE_DIR% -B %EXAMPLE_BUILD_DIR% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% || exit /b !ERRORLEVEL!
echo ----- SCPPLogger example's CMake configuration done -----

echo ----- Building SCPPLogger example in %EXAMPLE_BUILD_DIR% -----
cmake --build %EXAMPLE_BUILD_DIR% --config %BUILD_TYPE% || exit /b !ERRORLEVEL!
echo ----- SCPPLogger example's build done -----

echo ----- Installing SCPPLogger examples in %EXAMPLE_BUILD_DIR% -----
cmake --install %EXAMPLE_BUILD_DIR% --config %BUILD_TYPE% || exit /b !ERRORLEVEL!
